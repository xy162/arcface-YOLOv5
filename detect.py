            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or opt.save_crop or view_img:  # Add bbox to image
                        ...
                        #label = f'{names[int(cls)]} {conf:.2f}{"xxx"}'
                        if classify:
                            label123 = classifier_label(model=modelc, xyxy=xyxy, device=device, person_dir=person_dir,
                                                     im0=im0, features=features)
                            label = f'{names[int(cls)]}{conf:.2f}{label123}'
                        else:
                            label = None if opt.hide_labels else (
                                f'{names[int(cls)]}{conf:.2f}{"other1"}' if opt.hide_conf else f'{names[int(cls)]} {conf:.2f}{"other2"}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--yolo_weight', nargs='+', type=str, default='weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--arcface_weight', type=str, default=r'weights/arcface.pth',
                        help='arcface.pth path')
    parser.add_argument('--source', type=str, default='../1111.mp4', help='source')  # file/folder, 0 for webcam
